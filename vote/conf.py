...
from .exceptions import VoteConfigError
...
Settings.SECRET_KEY = getattr(
    project_settings, 'VOTE_SECRET_KEY', 'a-bad-secret-key')

Settings.AWS_ACCESS_KEY_ID = getattr(
    project_settings, 'VOTE_AWS_ACCESS_KEY_ID', None)

Settings.AWS_SECRET_ACCESS_KEY = getattr(
    project_settings, 'VOTE_AWS_SECRET_ACCESS_KEY', None)

Settings.AWS_REGION = getattr(
    project_settings, 'VOTE_AWS_REGION', None)

Settings.AWS_S3_BUCKET = getattr(
    project_settings, 'VOTE_AWS_S3_BUCKET', None)

Settings.CLOUDFRONT_ALTERNATE_DOMAIN = getattr(
    project_settings, 'VOTE_CLOUDFRONT_ALTERNATE_DOMAIN', None)

Settings.S3_UPLOAD_ROOT = getattr(
    project_settings, 'VOTE_S3_UPLOAD_ROOT', 'uploads/vote')
